...
import 'package:sqflite_shoping_list/constant.dart';
...
class DataClass {
  ...
  Future<int> delete(String name) async {
    ...
    return await dbready
        .rawDelete("DELETE FROM crudmodels WHERE ${Constant.name}='$name'");
  }

  Future<Crudmodel?> read(String name) async {
    var dbready = await db;
    var read = await dbready
        .rawQuery("SELECT * FROM crudmodels WHERE ${Constant.name}='$name'");
    if (read.isNotEmpty) {
